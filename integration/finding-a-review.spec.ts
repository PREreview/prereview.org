...
test.extend(canLogIn).extend(areLoggedIn)(
  'can see my own PREreviews',
  async ({ fetch, javaScriptEnabled, page, port }) => {
    ...
    fetch.get({
      ...
      query: {
        q: `(metadata.related_identifiers.resource_type.id:"publication-preprint" OR (metadata.related_identifiers.resource_type.id:"dataset" AND metadata.related_identifiers.identifier:/http:\\/\\/localhost:${port}\\/reviews\\/.+/)) AND (metadata.creators.person_or_org.identifiers.identifier:0000-0002-1825-0097 metadata.creators.person_or_org.name:"Orange Panda")`,
        size: '100',
...
test('can find and view a review', async ({ fetch, page }) => {
  ...
  fetch.get({
    ...
    response: { body: RecordsC.encode({ hits: { total: 1, hits: [record] } }) },
  })
...
test('might not load the older reviews in time', async ({ fetch, page }) => {
  fetch.modifyRoute('recent-prereviews', { delay: Duration.toMillis('5.5 seconds') })
