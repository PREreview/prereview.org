...
) => Either.Either<OrcidId.OrcidId, Queries.UnexpectedSequenceOfEvents> = flow(
  Array.findLast(hasTag('DatasetReviewWasStarted')),
  Option.map(Struct.get('authorId')),
  Either.fromOption(() => new Queries.UnexpectedSequenceOfEvents({ cause: 'No DatasetReviewWasStarted event found' })),
)
