...
export type Result = Either.Either<
  Option.Option<Pick<Events.AnsweredIfTheDatasetHasEnoughMetadata, 'answer' | 'detail'>>,
...
export const query = (events: ReadonlyArray<Events.DatasetReviewEvent>, input: Input): Result =>
  Either.gen(function* () {
    ...
    return Option.map(
      Array.findLast(filteredEvents, hasTag('AnsweredIfTheDatasetHasEnoughMetadata')),
