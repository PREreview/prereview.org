...
export class HasBeenDeclared extends Data.TaggedClass('HasBeenDeclared')<{
  competingInterests: Events.CompetingInterestsForADatasetReviewWereDeclared['competingInterests']
...
export const foldState = (events: ReadonlyArray<Events.DatasetReviewEvent>, datasetReviewId: Uuid.Uuid): State => {
  ...
  return Option.match(Array.findLast(filteredEvents, hasTag('DatasetReviewWasStarted')), {
    ...
    onSome: ({ authorId }) => {
      ...
      return Option.match(Array.findLast(filteredEvents, hasTag('CompetingInterestsForADatasetReviewWereDeclared')), {
        onNone: () => new NotDeclared({ authorId }),
        onSome: ({ competingInterests }) => new HasBeenDeclared({ competingInterests, authorId }),
      })
    },
  })
}
