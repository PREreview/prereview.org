...
const showDataPresentationForm = flow(
  fromReaderK(({ form, preprint, user }: { form: Form; preprint: PreprintTitle; user: User }) =>
    dataPresentationForm(preprint, { dataPresentation: E.right(form.dataPresentation) }, user),
  ),
...
const handleDataPresentationForm = ({ form, preprint, user }: { form: Form; preprint: PreprintTitle; user: User }) =>
  pipe(
    RM.decodeBody(body =>
      E.right({ dataPresentation: pipe(DataPresentationFieldD.decode(body), E.mapLeft(missingE)) }),
    ),
              ...
              ${E.isLeft(form.dataPresentation)
                ? html`
                    <div class="error-message" id="data-presentation-error">
                      ...
                      ${match(form.dataPresentation.left)
                        .with({ _tag: 'MissingE' }, () => 'Select if the data presentations are appropriate and clear')
                        .exhaustive()}
                    </div>
                  `
                : ''}
              ...
              <ol>
                ...
                <li>
                  ...
                  <p id="data-presentation-tip-neutral" role="note">
                    They effectively convey the necessary information, employ appropriate labeling, and utilize suitable
