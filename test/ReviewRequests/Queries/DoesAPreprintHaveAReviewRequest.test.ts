...
const requester2 = { name: Name.Name('Jean-Baptiste Botul') }
const requester3 = { name: Name.Name('Arne Saknussemm') }
const requester4 = { name: Name.Name('Axel Lidenbrock') }
...
test.each<[string, _.Input, ReadonlyArray<ReviewRequests.ReviewRequestEvent>, _.Result]>([
  ...
  const { initialState, updateStateWithEvents, query } = _.DoesAPreprintHaveAReviewRequest
