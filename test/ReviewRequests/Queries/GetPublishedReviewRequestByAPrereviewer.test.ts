...
import * as _ from '../../../src/ReviewRequests/Queries/GetPublishedReviewRequestByAPrereviewer.ts'
...
test.each<[string, _.Input, ReadonlyArray<ReviewRequests.ReviewRequestEvent>, _.Result]>([
  ...
  [
    'ready to be published with multiple choices',
    { requesterId: requesterId1, preprintId: preprintId1 },
  ...
  [
    'has been published with a persona choice',
    { requesterId: requesterId1, preprintId: preprintId1 },
    [started, published],
    Either.right(reviewRequestId1),
  ],
  ...
  const { initialState, updateStateWithEvents, query } = _.GetPublishedReviewRequestByAPrereviewer
