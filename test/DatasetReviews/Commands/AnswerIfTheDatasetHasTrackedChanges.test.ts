...
import * as _ from '../../../src/DatasetReviews/Commands/AnswerIfTheDatasetHasTrackedChanges.js'
...
const answered1 = new DatasetReviews.AnsweredIfTheDatasetHasTrackedChanges({ answer: 'no', datasetReviewId })
const answered2 = new DatasetReviews.AnsweredIfTheDatasetHasTrackedChanges({ answer: 'yes', datasetReviewId })
...
describe('foldState', () => {
  ...
  test.failing.prop(
    [
      fc
        .tuple(fc.datasetReviewWasStarted(), fc.datasetReviewAnsweredIfTheDatasetHasTrackedChanges())
...
describe('decide', () => {
  ...
  test.failing.prop([fc.constantFrom('yes', 'partly', 'no', 'unsure')])('has not been answered', answer => {
    ...
    expect(result).toStrictEqual(
      Either.right(Option.some(new DatasetReviews.AnsweredIfTheDatasetHasTrackedChanges({ answer, datasetReviewId }))),
    )
  })
  ...
  describe('has been answered', () => {
    test.failing.prop([
      ...
      expect(result).toStrictEqual(
        Either.right(
          Option.some(new DatasetReviews.AnsweredIfTheDatasetHasTrackedChanges({ answer: answer2, datasetReviewId })),
        ),
      )
    })
