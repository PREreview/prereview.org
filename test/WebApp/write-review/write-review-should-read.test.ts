...
describe('writeReviewShouldRead', () => {
    ...
    ([preprintId, preprintTitle, [shouldRead, shouldReadDetails, body], user, locale, newReview]) =>
      Effect.gen(function* () {
        ...
        const actual = yield* Effect.promise(
          _.writeReviewShouldRead({ body, locale, method: 'POST', id: preprintId, user })({
            formStore,
  ...
  it.effect.prop(
    ...
    ([preprintId, preprintTitle, body, method, user, locale]) =>
      Effect.gen(function* () {
        const actual = yield* Effect.promise(
          _.writeReviewShouldRead({ body, locale, method, id: preprintId, user })({
            formStore: new Keyv(),
            getPreprintTitle: () => TE.right(preprintTitle),
          }),
        )
  ...
  it.effect.prop(
    ...
    ([preprintId, preprintTitle, body, method, locale]) =>
      Effect.gen(function* () {
        ...
        const actual = yield* Effect.promise(
          _.writeReviewShouldRead({ body, locale, method, id: preprintId, user: undefined })({
            formStore,
            getPreprintTitle: () => TE.right(preprintTitle),
          }),
        )
