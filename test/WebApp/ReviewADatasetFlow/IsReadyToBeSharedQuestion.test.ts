...
import * as _ from '../../../src/WebApp/ReviewADatasetFlow/IsReadyToBeSharedQuestion/index.ts'
  ...
  test.prop([fc.uuid(), fc.supportedLocale(), fc.user()])(
    ...
    (datasetReviewId, locale, user) =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.mock(DatasetReviews.DatasetReviewQueries, {
            checkIfUserCanAnswerIfTheDatasetIsReadyToBeShared: () => new DatasetReviews.UnableToQuery({}),
          }),
        ),
        Effect.provideService(Locale, locale),
        Effect.provideService(LoggedInUser, user),
...
describe('IsReadyToBeSharedSubmission', () => {
  describe('when there is an answer', () => {
    describe('when the answer can be saved', () => {
      test.prop([
        ...
        Effect.gen(function* () {
          const actual = yield* _.IsReadyToBeSharedSubmission({ body, datasetReviewId })
          ...
          Effect.provide(
            Layer.mock(DatasetReviews.DatasetReviewCommands, {
              answerIfTheDatasetIsReadyToBeShared: () => Effect.void,
            }),
          ),
          Effect.provide(
            Layer.mock(DatasetReviews.DatasetReviewQueries, {
