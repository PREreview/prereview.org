...
import * as _ from '../../../src/WebApp/ReviewADatasetFlow/FollowsFairAndCarePrinciplesQuestion/index.ts'
  ...
  it.effect.prop(
    ...
    ([datasetReviewId, locale, user]) =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.mock(DatasetReviews.DatasetReviewQueries, {
            checkIfUserCanAnswerIfTheDatasetFollowsFairAndCarePrinciples: () => new Queries.UnableToQuery({}),
          }),
        ),
        Effect.provideService(Locale, locale),
        Effect.provideService(LoggedInUser, user),
  ...
  it.effect.prop(
    ...
    [
      ...
      fc.oneof(
        fc
          .urlParams()
          .filter(urlParams => Option.isNone(UrlParams.getFirst(urlParams, 'followsFairAndCarePrinciples'))),
        fc.urlParams(
          fc.record(
            {
