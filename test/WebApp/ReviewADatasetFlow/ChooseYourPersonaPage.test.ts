...
describe('ChooseYourPersonaPage', () => {
  describe('when the dataset review is by the user', () => {
    describe('when the dataset review is in progress', () => {
      it.effect.prop(
        'when the personas can be loaded',
        [
          fc.uuid(),
          fc.supportedLocale(),
          fc.user(),
        ...
        ([datasetReviewId, locale, user, chosen, publicPersona, pseudonymPersona]) =>
          Effect.gen(function* () {
      ...
      it.effect.prop(
        "when a persona can't be loaded",
        [fc.uuid(), fc.supportedLocale(), fc.user(), fc.maybe(fc.constantFrom('public', 'pseudonym')), fc.anything()],
        ([datasetReviewId, locale, user, chosen, error]) =>
          Effect.gen(function* () {
            const actual = yield* _.ChooseYourPersonaPage({ datasetReviewId })
...
describe('ChooseYourPersonaSubmission', () => {
  describe('when there is an choice', () => {
    ...
    it.effect.prop(
      "when the choice can't be saved",
      [
        fc.uuid(),
        fc.urlParams(fc.record({ chooseYourPersona: fc.constantFrom('public', 'pseudonym') })),
        fc.supportedLocale(),
  ...
  describe("when there isn't an choice", () => {
    it.effect.prop(
      ...
      ([datasetReviewId, body, locale, user, publicPersona, pseudonymPersona]) =>
        Effect.gen(function* () {
          const actual = yield* _.ChooseYourPersonaSubmission({ body, datasetReviewId })
    ...
    it.effect.prop(
      "when the personas can't be loaded",
      [
        fc.uuid(),
        fc.oneof(
          fc.urlParams().filter(urlParams => Option.isNone(UrlParams.getFirst(urlParams, 'chooseYourPersona'))),
