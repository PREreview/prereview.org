...
describe('reviewPage', () => {
  it.effect.prop(
    ...
    [
      ...
      fc.array(
        fc.record({
          authors: fc.record({
            named: fc.nonEmptyArray(fc.record({ name: fc.name(), orcid: fc.orcidId() }, { requiredKeys: ['name'] })),
          }),
          doi: fc.doi(),
          id: fc.integer(),
  ...
  it.effect.prop('when the review is not found', [fc.supportedLocale(), fc.integer(), fc.boolean()], ([locale, id]) =>
    Effect.gen(function* () {
      const actual = yield* Effect.promise(
        _.reviewPage({ id, locale })({
  ...
  it.effect.prop('when the review was removed', [fc.supportedLocale(), fc.integer()], ([locale, id]) =>
    Effect.gen(function* () {
      const actual = yield* Effect.promise(
        _.reviewPage({ id, locale })({
  ...
  it.effect.prop('when the review cannot be loaded', [fc.supportedLocale(), fc.integer()], ([locale, id]) =>
    Effect.gen(function* () {
      const actual = yield* Effect.promise(
        _.reviewPage({ id, locale })({
  ...
  it.effect.prop(
    ...
    [fc.supportedLocale(), fc.integer(), fc.prereview()],
    ([locale, id, prereview]) =>
      Effect.gen(function* () {
        const actual = yield* Effect.promise(
          _.reviewPage({ id, locale })({
