...
import { type Name, NameSchema } from '../src/types/Name.ts'
...
export const prereview = ({
  ...
  fc
    .record({
      authors: fc.record({
        named: nonEmptyArray(fc.record({ name: name(), orcid: orcidId() }, { requiredKeys: ['name'] })),
...
export const invitationToAppearOnADatasetReviewAddedToTheList = ({
  datasetReviewId,
}: {
  datasetReviewId?: fc.Arbitrary<Events.InvitationToAppearOnADatasetReviewAddedToTheList['datasetReviewId']>
} = {}): fc.Arbitrary<Events.InvitationToAppearOnADatasetReviewAddedToTheList> =>
  fc
    .record({
      datasetReviewId: datasetReviewId ?? uuid(),
      invitationId: uuid(),
...
export const datasetReviewWasPublished = ({
  ...
  fc
    .record({ datasetReviewId: datasetReviewId ?? uuid(), publicationDate: fc.oneof(instant(), plainDate()) })
...
export const reviewRequestForAPreprintWasRecategorized = ({
  reviewRequestId,
}: {
  reviewRequestId?: fc.Arbitrary<Events.ReviewRequestForAPreprintWasRecategorized['reviewRequestId']>
} = {}): fc.Arbitrary<Events.ReviewRequestForAPreprintWasRecategorized> =>
  fc
    .record(
      {
        reviewRequestId: reviewRequestId ?? uuid(),
      ...
      { requiredKeys: ['reviewRequestId'] },
    )
    .map(data => new Events.ReviewRequestForAPreprintWasRecategorized(data))
...
export const reviewRequestEvent = (
  ...
  fc.oneof(
    ...
    reviewRequestForAPreprintWasRecategorized(args),
...
export const emailToInviteAuthorSent = (): fc.Arbitrary<Events.EmailToInviteAuthorSent> =>
  fc
    .record({
      invitationId: uuid(),
      sentAt: instant(),
    })
    .map(args => new Events.EmailToInviteAuthorSent(args))
