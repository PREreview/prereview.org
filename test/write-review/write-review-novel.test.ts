...
describe('writeReviewNovel', () => {
  test.prop([
    ...
    fc.preprintTitle(),
    fc.tuple(fc.novel(), fc.novelDetails()).chain(([novel, novelDetails]) =>
      fc.tuple(
        fc.constant(novel),
        fc.constant(novelDetails),
        fc.connection({
          body: fc.constant({
            novel,
    ...
    async (preprintId, preprintTitle, [novel, novelDetails, connection], user, locale, newReview) => {
