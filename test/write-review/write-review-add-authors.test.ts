...
describe('writeReviewAddAuthors', () => {
  test.prop([
    ...
    const actual = await _.writeReviewAddAuthors({ id: preprintId, method: 'POST', user })({
      formStore,
  ...
  test.prop([fc.indeterminatePreprintId(), fc.preprintTitle(), fc.string(), fc.user()])(
    'when there is no form',
    async (preprintId, preprintTitle, method, user) => {
      const actual = await _.writeReviewAddAuthors({ id: preprintId, method, user })({
        formStore: new Keyv(),
        getPreprintTitle: () => TE.right(preprintTitle),
      })()
  ...
  ])('when there are no more authors', async (preprintId, preprintTitle, method, user, newReview) => {
    const formStore = new Keyv()
    await formStore.set(formKey(user.orcid, preprintTitle.id), FormC.encode(newReview))
  ...
  test.prop([fc.indeterminatePreprintId(), fc.preprintTitle(), fc.string()])(
    "when there isn't a session",
    async (preprintId, preprintTitle, method) => {
      const actual = await _.writeReviewAddAuthors({ id: preprintId, method })({
        formStore: new Keyv(),
        getPreprintTitle: () => TE.right(preprintTitle),
      })()
