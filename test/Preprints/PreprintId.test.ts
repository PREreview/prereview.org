...
describe('PreprintIdEquivalence', () => {
  it.prop(
    'with the same preprint ID',
    [fc.indeterminatePreprintId().map(id => [id, id] as const)],
    ([[id1, id2]]) => {
...
describe('isPreprintDoi', () => {
  it.prop('with a preprint DOI', [fc.preprintDoi()], ([doi]) => {
    expect(_.isPreprintDoi(doi)).toBe(true)
  })

  it.prop('with a non-preprint DOI', [fc.doi(fc.constantFrom('0001', '1', '123', '1000'))], ([doi]) => {
...
describe('fromUrl', () => {
  it.prop(
    'with a doi.org URL',
    [fc.preprintDoi().map(doi => [toUrl(doi), doi] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an AfricArXiv URL',
    [fc.africarxivPreprintUrl().map(([url, id]) => [url, Array.of(id)] as const)],
    ([[url, ids]]) => {
  ...
  it.prop(
    'with an arxiv.org URL',
    [fc.arxivPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an authorea.com URL',
    [fc.authoreaPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with a biorxiv.org URL',
    [fc.biorxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
  ...
  it.prop(
    'with an chemrxiv.org URL',
    [fc.chemrxivPreprintUrl()],
    ([url]) => {
  ...
  it.prop(
    'with an eartharxiv.org URL',
    [fc.eartharxivPreprintUrl()],
    ([url]) => {
      expect(_.fromUrl(url)).toHaveLength(0)
    },
    {
  ...
  it.prop(
    'with an ecoevorxiv.org URL',
    [fc.ecoevorxivPreprintUrl()],
    ([url]) => {
      expect(_.fromUrl(url)).toHaveLength(0)
    },
    {
  ...
  it.prop(
    'with an edarxiv.org URL',
    [fc.edarxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
  ...
  it.prop(
    'with an engrxiv.org URL',
    [fc.engrxivPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an Lifecycle Journal URL',
    [fc.lifecycleJournalPreprintUrl()],
    ([[url, ids]]) => {
      expect(_.fromUrl(url)).toStrictEqual(ids)
    },
    {
  ...
  it.prop(
    'with a medrxiv.org URL',
    [fc.medrxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
      ...
      fastCheck: {
        examples: [
          ...
          [
            [
              new URL(
                'https://www.medrxiv.org/content/medrxiv/early/2023/02/17/2023.01.17.23284673/embed/graphic-3.gif',
              ), // article thumbnail
  ...
  it.prop(
    'with a MetaArXiv URL',
    [fc.metaarxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
  ...
  it.prop(
    'with an neurolibre.org URL',
    [fc.neurolibrePreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
    ...
    },
    {
      fastCheck: {
        examples: [
          ...
          [
            [
              new URL('https://preprint.neurolibre.org/10.55458/neurolibre.00031.pdf'),
              Doi('10.55458/neurolibre.00031'),
            ],
          ], // pdf
          [
  ...
  it.prop(
    'with an OSF Preprints URL',
    [fc.osfPreprintsPreprintUrl()],
    ([[url, ids]]) => {
      expect(_.fromUrl(url)).toStrictEqual(ids)
    },
    {
  ...
  it.prop(
    'with a philsci-archive.pitt.edu URL',
    [fc.philsciPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, id]]) => {
    ...
    },
    {
      fastCheck: {
        examples: [
          ...
          [
            [
              new URL('https://philsci-archive.pitt.edu/21986/1.haslightboxThumbnailVersion/preprint_OS_2023.pdf'),
              21986,
            ],
          ], // preview
          ...
          [
            [
              new URL('https://philsci-archive.pitt.edu/cgi/export/21986/Text_Chicago/philsci-archive-21986.txt'),
              21986,
            ],
          ], // text citation
        ],
  ...
  it.prop(
    'with an preprints.org URL',
    [fc.preprintsorgPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an psyarxiv.com URL',
    [fc.psyarxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
  ...
  it.prop(
    'with a researchsquare.com URL',
    [fc.researchSquarePreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with a SciELO URL',
    [fc.scieloPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with a scienceopen.com URL',
    [fc.scienceOpenPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an SocArXiv URL',
    [fc.socarxivPreprintUrl().map(([url, id]) => [url, Array.of(id.value)] as const)],
    ([[url, dois]]) => {
  ...
  it.prop(
    'with a SSRN URL',
    [fc.ssrnPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with an techrxiv.org URL',
    [fc.techrxivPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
  ...
  it.prop(
    'with a Zenodo URL',
    [fc.zenodoPreprintUrl().map(([url, id]) => [url, id.value] as const)],
    ([[url, doi]]) => {
