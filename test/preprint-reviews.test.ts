...
describe('preprintReviews', () => {
  test.prop([
    ...
    async (publicUrl, connection, user, preprint, prereviews, rapidPrereviews, canSeeClubs) => {
  ...
  test.prop([
    ...
    fc.boolean(),
  ])('when the preprint is not found', async (publicUrl, connection, user, preprintId, canSeeClubs) => {
    const actual = await runMiddleware(
  ...
  test.prop([
    ...
    fc.indeterminatePreprintId(),
    fc.boolean(),
  ])('when the preprint is unavailable', async (publicUrl, connection, user, preprintId, canSeeClubs) => {
    const actual = await runMiddleware(
      _.preprintReviews(preprintId)({
        canSeeClubs,
  ...
  test.prop([
    ...
    async (publicUrl, connection, user, preprint, rapidPrereviews, canSeeClubs) => {
      const actual = await runMiddleware(
        _.preprintReviews(preprint.id)({
          canSeeClubs,
  ...
  test.prop([
    ...
    async (publicUrl, connection, user, preprint, prereviews, canSeeClubs) => {
      const actual = await runMiddleware(
        _.preprintReviews(preprint.id)({
          canSeeClubs,
          getPreprint: () => TE.right(preprint),
