...
import * as _ from '../../src/AuthorInvites/GetAuthorChoicesToConfirm.ts'
...
test.each<[string, _.Input, ReadonlyArray<Events.Event>, _.Result]>([
  ...
  ['accepted an invite', input, [added, accepted], Either.left(new _.PersonaHasNotBeenChosen())],
  [
  ...
  ['started the review', input, [started, added, accepted], Either.left(new _.ChoicesHaveBeenConfirmed())],
  [
    'different ORCID iD started the review',
    input,
    [startedDifferentOrcidId, added, accepted],
    Either.left(new _.PersonaHasNotBeenChosen()),
  ],
  ['persona chosen', input, [added, accepted, persona], Either.right({ persona: persona.persona })],
  ['details confirmed', input, [added, accepted, persona, confirmed], Either.left(new _.ChoicesHaveBeenConfirmed())],
])('%s', (_name, input, events, expected) => {
  const { query } = _.GetAuthorChoicesToConfirm
