...
describe('authorInvite', () => {
  test.prop([
  ...
  ])('when the review can be loaded', async (inviteId, invite, prereview) => {
    ...
    const actual = await _.authorInvite(inviteId)({
      getAuthorInvite,
      getPrereview,
    })()
  ...
  test.prop([fc.uuid(), fc.authorInvite()])('when the review cannot be loaded', async (inviteId, invite) => {
    const actual = await _.authorInvite(inviteId)({
  ...
  test.prop([fc.uuid()])('when the invite cannot be loaded', async inviteId => {
    const actual = await _.authorInvite(inviteId)({
