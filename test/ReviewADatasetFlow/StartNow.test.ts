...
describe('StartNow', () => {
  describe("a review hasn't been started", () => {
    describe('the review can be started', () => {
      test.prop([fc.supportedLocale(), fc.user(), fc.datasetTitle(), fc.uuid(), fc.datasetReviewNextExpectedCommand()])(
        'the next expected command can be found',
        (locale, user, dataset, uuid, nextExpectedCommand) =>
          Effect.gen(function* () {
      ...
      ])("the next expected command can't be found", (locale, user, dataset, uuid, result) =>
        Effect.gen(function* () {
          const actual = yield* _.StartNow
    ...
    ])("the review can't be started", (locale, user, dataset, uuid, error) =>
      Effect.gen(function* () {
        const actual = yield* _.StartNow
  ...
  describe('a review has been started', () => {
    test.prop([fc.supportedLocale(), fc.user(), fc.datasetTitle(), fc.uuid(), fc.datasetReviewNextExpectedCommand()])(
      'the next expected command can be found',
      (locale, user, dataset, reviewId, nextExpectedCommand) =>
        Effect.gen(function* () {
          const actual = yield* _.StartNow
    ...
    ])("the next expected command can't be found", (locale, user, dataset, reviewId, result) =>
      Effect.gen(function* () {
        const actual = yield* _.StartNow
  ...
  test.prop([fc.supportedLocale(), fc.user(), fc.datasetTitle(), fc.anything()])(
