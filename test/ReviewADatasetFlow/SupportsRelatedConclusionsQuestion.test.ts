...
import * as _ from '../../src/ReviewADatasetFlow/SupportsRelatedConclusionsQuestion/index.js'
  ...
  test.prop([fc.uuid(), fc.supportedLocale(), fc.user()])(
    ...
    (datasetReviewId, locale, user) =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.mock(DatasetReviews.DatasetReviewQueries, {
            checkIfUserCanAnswerIfTheDatasetSupportsRelatedConclusions: () => new DatasetReviews.UnableToQuery({}),
          }),
        ),
        Effect.provideService(Locale, locale),
        Effect.provideService(LoggedInUser, user),
...
test.prop([fc.uuid(), fc.urlParams(), fc.supportedLocale(), fc.user()])(
  'SupportsRelatedConclusionsSubmission',
