...
import * as _ from '../../src/ReviewADatasetFlow/HasDataCensoredOrDeletedQuestion/index.ts'
  ...
  test.prop([fc.uuid(), fc.supportedLocale(), fc.user()])(
    ...
    (datasetReviewId, locale, user) =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          Layer.mock(DatasetReviews.DatasetReviewQueries, {
            checkIfUserCanAnswerIfTheDatasetHasDataCensoredOrDeleted: () => new DatasetReviews.UnableToQuery({}),
          }),
        ),
        Effect.provideService(Locale, locale),
        Effect.provideService(LoggedInUser, user),
