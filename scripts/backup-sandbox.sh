...
for key in $(redis-cli -p 16379 -a "$REDIS_PASSWORD" --no-auth-warning --scan --pattern '*'); do
  ...
  if [ "$type" = "string" ]; then
    redis-cli -p 16379 -a "$REDIS_PASSWORD" --no-auth-warning GET "$key" | jq -c --arg k "$key" '{key: $k} + .' || echo "Failed to parse output for key: $key" >&2
  elif [ "$type" = "set" ]; then
...
age \
  ...
  -o "backup/${APP}-backup-${TIMESTAMP}.tar.gz.age" \
