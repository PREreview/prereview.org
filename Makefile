...
test-integration: test-integration-image
	...
	docker run --rm --env "COCKROACHDB_URL=postgres://prereview:@host.docker.internal:26257/defaultdb" --volume "$$(pwd)"/integration-results:/app/integration-results --volume "$$(pwd)"/visual-regression/snapshots:/app/visual-regression/snapshots ${INTEGRATION_TEST_IMAGE_TAG} ${TEST} ${ARGS}
